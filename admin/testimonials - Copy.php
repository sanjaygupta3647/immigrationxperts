...
function insertTestimonial($clientName, $clientLocation, $clientQuote, $userId, $clientImage, $facebookLink, $linkedinLink) {
    global $conn;

    $submissionDate = date("Y-m-d H:i:s");

    $stmt = $conn->prepare("INSERT INTO testimonials (client_name, client_location, client_quote, submission_date, user_id, client_image, facebook_link, linkedin_link) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssss", $clientName, $clientLocation, $clientQuote, $submissionDate, $userId, $clientImage, $facebookLink, $linkedinLink);
...
function updateTestimonial($testimonialId, $clientName, $clientLocation, $clientQuote, $clientImage, $facebookLink, $linkedinLink) {
    global $conn;

    $stmt = $conn->prepare("UPDATE testimonials SET client_name=?, client_location=?, client_quote=?, client_image=?, facebook_link=?, linkedin_link=? WHERE id=?");
    $stmt->bind_param("ssssssi", $clientName, $clientLocation, $clientQuote, $clientImage, $facebookLink, $linkedinLink, $testimonialId);
...
if (isset($_GET['edit'])) {
    $editTestimonialId = $_GET['edit'];
    $editTestimonial = getTestimonialById($editTestimonialId);

    if ($editTestimonial) {
        // Populate form fields with existing data
        $editClientName = $editTestimonial['client_name'];
        $editClientLocation = $editTestimonial['client_location'];
        $editClientQuote = $editTestimonial['client_quote'];
        $editClientImage = $editTestimonial['client_image'];
        $editFacebookLink = $editTestimonial['facebook_link'];
        $editLinkedinLink = $editTestimonial['linkedin_link'];
    }
}

// Display the form (used for both add and edit)
echo "<h2>" . (isset($editTestimonial) ? "Edit" : "Add") . " Testimonial</h2>";
echo "<form action='{$_SERVER['PHP_SELF']}' method='post' enctype='multipart/form-data'>";
echo "<input type='hidden' name='action' value='" . (isset($editTestimonial) ? 'update' : 'insert') . "'>";
if (isset($editTestimonial)) {
    echo "<input type='hidden' name='testimonialId' value='$editTestimonialId'>";
}

// Function to display testimonials with pagination (ordered by ID in descending order)
function getTestimonials($page, $pageSize) {
    global $conn;
...
function searchTestimonials($searchTerm) {
    global $conn;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    switch ($action) {
        case 'insert':
            ...
            // Insert testimonial
            insertTestimonial($clientName, $clientLocation, $clientQuote, $userId, $clientImage, $facebookLink, $linkedinLink);
            break;
        ...
        case 'update':
            ...
            // Update testimonial
            updateTestimonial($testimonialId, $clientName, $clientLocation, $clientQuote, $clientImage, $facebookLink, $linkedinLink);
            break;
        ...
        case 'search':
            ...
            // Perform search
           $searchResults = searchTestimonials($searchTerm);
            break;
                            ...
                            <div class="card custom-card">
                                ...
                                <div class="card-body">
                                    <div class="row gy-4">
										...
										<input type='hidden' name='action' value='<?php echo isset($editTestimonial) ? 'update' : 'insert'; ?>'><div class="col-xl-12">
										...
										<button type='submit' class='btn btn-primary'><?php echo isset($editTestimonial) ? 'Update' : 'Submit'; ?> Testimonial</button>
										</form>
                        ...
                        <div class="col-xl-12">
                            <div class="card custom-card">
                                ...
                                <div class="card-body">
									...
									$displayResults = isset($searchResults) ? $searchResults : getTestimonials(1, getTotalTestimonials());
									...
									if (!empty($displayResults)) {
										...
										foreach ($displayResults as $result) {
											...
											echo "<input type='hidden' name='testimonialId' value='{$result['id']}'>";
								    ...
								    <!-- Display total count -->
								<div class="mb-3">
									<p>Total Testimonials: <?php echo getTotalTestimonials(); ?></p>
