...
if (!empty($_POST)) {
    ...
    $bodyMessage .= "<tr><td>Education: </td><td>".$postData['edu']."</td></tr>";
    $bodyMessage .= "<tr><td>Experience: </td><td>".$postData['exp']."</td></tr>";
    ...
    // Set up recipients
    $mail->setFrom('noida@immigrationxperts.com', 'Australia YT'); // Set the sender
    $mail->addAddress('noida@immigrationxperts.com', 'Australia YT'); // Add a recipient
    ...
    $mail->Subject = "Australia YT : " . $canonical_url; // Set the subject of the email
    $mail->Body    = $bodyMessage; // Set the body of the email
    
    // Send the email
    try {
        $mail->send();
        echo '<script>window.location.href = "/australia-pr/yt/thank-you/";</script>';
    } catch (Exception $e) {
